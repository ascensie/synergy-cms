...
const DATA_DIR = path_1.default.resolve(__dirname, '../../synergy-website/data');
// Helper function to dynamically import data files
async function importData(filename) {
    try {
        const module = await Promise.resolve(`${path_1.default.join(DATA_DIR, filename)}`).then(s => __importStar(require(s)));
        return module;
    }
    catch (error) {
